  ...
  */
    function register($szLogin='',$szPassword='') {
        ...
        $error_message = "";
         ...
         //find has for submitted password
         $untested_hash = password_hash($szPassword, PASSWORD_BCRYPT);
         //verify hash
         if (password_verify($szPassword, $untested_hash)) {
         	$szHash=$untested_hash;
         	$query = "SELECT * FROM $mysql_user_table WHERE (" . $mysql_user_select_login . "='" . $szLogin . $mysql_user_append_to_login . "' or " . $mysql_user_select_login . "='" . $szLogin . "') . and 'stored_hash' . "=" . $szhash . "' AND aliased_name IS NULL";
         	if(!$result) {
         ...
         if(mysql_num_rows($result) <= 0 ) {
         	...
         	$error_message = "Login failded: has could not be verified.";
		...
		*/
         ...
         $_SESSION['hash'] = $szHash;
         $_SESSION['IPP_double_login'] = TRUE; //I don't know
 ...
 */
    ...
    function validate($szLogin='',$szPassword='') {
         ...
         //check session logged in...checks the database for current logins
