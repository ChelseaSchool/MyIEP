...
require_once 'page_troubleshoot.php';
 ...
 *
 * @param string $szLogin       Parameter name arbitrary?
 *
 * @param string $szPassword    Parameter name arbitrary?
 *
...
function register($szLogin='', $szPassword='')
{
    ...
    $_SESSION['hash'] = $hash;
    $_SESSION['IPP_double_login'] = false;
    if (!connectIPPDB()) {
...
/** @fn validate()
 *
 * @param string $szLogin    Seemingly arbitrary name
 *
 * @param string $szPassword Seemingly arbitrary name
 *
 * @return bool
 *
 *
 ...
 * @param string $szUsername get permission level for username from db
 * 
 ...
 * 
 * @brief sets permissions for lowest authentication level - unless otherwise specified
 * 
 * @param int $PERMISSION_LEVEL = 100
 * 
 * @return $retval
 * 
 * @todo 
 * untangle this mess. Refactor as necessary.
 * @bug
 *
 */
 ...
 *
 * @param string $student_id from db
 * 
 * @return string
 * 
 * @todo
