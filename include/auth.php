 ...
 * @details
 * Starts session if login is successful.
 * Throws errors and returns false if:
 ...
 *
 * @return boolean
 * @todo 1. Add are you sure alert and ask for confirmation for logout
 *      
 */
function logout() {
 ...
 * @brief Begins session, checks username and password against user table in MySQL.
 * Returns false and throws error message if there's a problem.
 *
 * @param string $szLogin        	
 * @param string $szPassword        	
 * @return boolean
...
function get_services($PERMISSION_LEVEL = 100) {
	...
	if (! $system_result) {
		...
		if ($system_row ['is_local_ipp_administrator'] == 'Y')
			$is_local_ipp_administrator = TRUE;
	}
 ...
 * @detail
 * 1.
 * Returns error or null under some circumstances.
 ...
 * @todo 1. Rename function because it is a confusing name
