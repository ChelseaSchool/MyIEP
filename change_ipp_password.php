...
if (isset ( $_GET ['username'] ))
	$ippuserid = mysql_real_escape_string ( $_GET ['username'] );
else if (isset ( $_POST ['username'] ))
	$ippuserid = mysql_real_escape_string ( $_POST ['username'] );
else
	$ippuserid = $_SESSION ['egps_username'];
...
if (($_SESSION ['egps_username'] != $ippuserid) && (isLocalAdministrator ( $_SESSION ['egps_username'] ) && getPermissionLevel ( $_SESSION ['egps_username'] ) > $MINIMUM_AUTHORIZATION_LEVEL)) {
	...
	if (! $user_result) {
		...
		if (mysql_num_rows ( $user_result ) <= 0)
			"IPP Member not found<BR>Query=$user_query";
	...
	if (! $us_result) {
		...
		if (mysql_num_rows ( $us_result ) <= 0)
			$system_message .= "IPP Member not found<BR>Query=$us_query";
...
if (! $user_result) {
	...
	if (mysql_num_rows ( $user_result ) <= 0)
		$system_message .= "IPP Member not found<BR>";
	$user_row = mysql_fetch_array ( $user_result );
}
...
?>
<h2>
            Enter &amp; Confirm New Password <small>and click Update</small>
        </h2>

        <div class="alert alert-block alert-info">
            <a href="#" class="close" data-dismiss="alert">&times;</a><strong>Notice</strong>:
            For password support, look for the question mark icon beside
            the password field.
        </div>

        <!-- Caps Lock Alert, for Now -->
        <div class="alert alert-danger" id="capsWarning"
            style="display: none;">Caps Lock is on.</div>

        <form enctype="multipart/form-data"
            action="<?php echo IPP_PATH . "change_ipp_password.php"; ?>"
            method="post">
            <input type="hidden" name="username"
                value="<?php echo $user_row['egps_username']; ?>">

            <div class="form-group">
                <label>Username</label> <input type="text"
                    class="form-control"
                    value="<?php echo $user_row['egps_username']; ?>"
                    disabled name="userid"> <label>Password</label>
                <div class="input-group">

                    <input class="form-control" type="password"
                        name="pwd1" size="30" maxsize="30" tabindex="1"
                        required
                        pattern="(?=^.{6,30}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[!@#$%^&*])(?=.*[A-Z])(?=.*[a-z]).*$"
                        placeholder="Please enter a complex password"><span
                        class="input-group-addon" data-toggle="modal"
                        data-target="#pw_support">?</span>
                </div>

                <label>Password (retype)</label> <input
                    class="form-control" type="password" required
                    name="pwd2" size="30" maxsize="30" tabindex="2"
                    required
                    pattern="(?=^.{6,30}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[!@#$%^&*])(?=.*[A-Z])(?=.*[a-z]).*$"
                    placeholder="Please confirm password">
            </div>
            <input type="hidden" required name="szBackGetVars"
                value="<?php echo $szBackGetVars; ?>">

            <button class="btn btn-default btn-large" type="submit"
                name="Update" value="Update" tabindex="3">Update</button>
        </form>

    </div>

    <!-- Modal-->
    <div class="modal fade" id="pw_support" tabindex="-1" role="dialog"
        aria-labelledby="options" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Please choose a strong password. Passwords for
                        MyIEP must include:</p>
                    <ul>
                        <li>At least one capital letter;</li>
                        <li>At least one lower-case letter;</li>
                        <li>At least one numeral;</li>
                        <li>At least one special character (keyboard
                            symbol);</li>
                    ...
                    <ul>
                        <li><a target="_new"
                            href="https://www.microsoft.com/en-gb/security/pc-security/password-checker.aspx">Password
                                Check</a> <small>A Microsoft site to
                                help evaluate passwords</small></li>
                        <li><a target="_new"
                            href="http://blog.kaspersky.com/password-check/">Password
                                Evaluator</a> <small>Very helpful
                                password evaluation tool from Kaspersky</small></li>
                        <li>Click <a target="_new"
                            href="https://infamia.com/hints/pwgen.php?length=10&quiet">here</a>
