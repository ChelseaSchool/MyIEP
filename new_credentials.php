...
<body>
	...
	<div class="container">
		...
		<form method="post" name="person_confirmer" id="confirmer" role="form"
			enctype="multipart/form-data" action="new_credentials_receipt.php"
			method="post">
			<input type="datetime" hidden name="date" required
				value="<?php echo date("Y-M-d");?>"> <input type="hidden"
				...
				type="hidden" name="sess" required
				value="<?php echo session_id(); ?>">
			<div class="form-group">

				<label>E-Mail Address</label> <input autocomplete="off"
					class="form-control email" type="email" required id="email1"
					name="email1"
					placeholder="Please enter the email address associated with your account.">
				<label>E-Mail Confirmation</label> <input autocomplete="off"
					class="form-control email" type="email" required id="email2"
					name="email2" placeholder="Please confirm your email address."> <label>Username</label>
				<input autocomplete="off" class="form-control" type="text" required
					id="uname" name="uname" placeholder="Please enter your username...">
				<label>Are you human? Solve <?php echo $integer1 . " + " . $integer2;?>
				</label> <input id="humanizer"
					pattern="<?php echo $calculated_sum; ?>" value=""
					autocomplete="off" class="form-control" type="text" required
					name="humanizer" placeholder="Please be human.">
			</div>
			<button role="button" id="validate" class="btn btn-default btn-lg"
				type="submit">Validate</button>
			<div class="alert alert-block alert-danger" id="mustmatch">
				<!-- <a href="#" class="close" data-dismiss="alert">&times;</a>-->
				<strong>Alert</strong>: E-mail fields must match & equation must be
				solved correctly.
			</div>
